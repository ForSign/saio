...
def max_way(graph, s, t):
    topological_list = topological_sort(graph, s, t)
    if PRINT_L:
        print('Sorted: {}'.format(topological_list))
    ...
    m_weight = list(start_list_way.values())[-1]
    if PRINT_L:
        print('Max weight: ', m_weight)
    ...
    path.reverse()
    if PRINT_L:
        print('OPT: ' + ' - '.join(str(x) for x in path))

    return (m_weight, path)
